...
\begin{frame}{Caracteres}
	...
	caracter $\theta$ módulo $r$ tales que $\chi(a) = \theta(a)$ para todo $a$ coprimo a $n$.)
\end{frame}
\begin{frame}{Solución}
	Claramente $\chi\cdot\psi$ es caracter módulo $nm$.
	Si no fuera primitivo, existe $r \mid nm$ y un caracter $\theta$ módulo $r$ tal que $\chi(a)\psi(a) = \theta(a)$ para $(a; n) = 1$.
	\pause
	...
	\pmod{m'}$) de modo que $\chi(b) = \theta(b) = \chi'(b)$, y análogamente se verifica que $\psi(a) = \psi'(a)$ para $(a; m) = 1$;
	esto contradice la primitividad de $\chi$ y $\psi$.
\end{frame}
...
\begin{frame}{Series de Dirichlet}
	\begin{block}{Problema}
		Pruebe que
		\[
			\frac{1}{x} \sum_{n\le x} \sigma(n) = \frac{\pi^2}{12}x^2 + O(x\log x).
